...
Here we are loading in the data and necessary libraries. In addition, we are eliminating approximately 10% of data, Start date is 2020-01-1 and the end date is 2020-04-23, cutoff for 90% of total data is 2020-04-12.
...
subsetfile <- subset(file, dateRep >= as.POSIXct('2020-01-1') & dateRep 
            <= as.POSIXct('2020-04-12'))
...
# log tranformation
US_series.ln = log(US_series)
plot(US_series.ln,type = 'l')

# log tranformation first difference
US_series.ln.diff1 = diff(US_series.ln)
plot(US_series.ln.diff1, type='l')

# log tranformation second difference
US_series.ln.diff2 = diff(US_series.ln.diff1)
plot(US_series.ln.diff2, type='l')

###-------------------------------------------------


US_series.ln.diff2 <- US_series.ln.diff2[!is.na(US_series.ln.diff2)]
US_series.ln.diff2 <- US_series.ln.diff2[-1]
 

# checking for autocorrelation
Box.test(US_series.ln.diff2, type = "Ljung-Box")
# checking stationarity of second ln difference 
adf.test(US_series.ln.diff2, alternative = "stationary")
...
acf2(US_series.ln.diff2, stationary = TRUE, seasonal = FALSE)

# fitting several models

MA1_diff_2 = sarima(US_series.ln.diff2, p=0, d=0, q=1)
MA1_diff_2

AR1_diff_2 = sarima(US_series.ln.diff2, p=1, d=0, q=0)
AR1_diff_2

AR4_MA3_diff_2 = sarima(US_series.ln.diff2, p=4, d=0, q=3)
AR4_MA3_diff_2
...
sarima.for(US_series.ln.diff2, n.ahead=5, p=4, d=0, q=3, P=0, D=0, Q=0, S=0)
US_series.ln = US_series.ln[-(1:15)]
sarima.for(US_series.ln, n.ahead=12, p=4, d=2, q=3, P=0, D=0, Q=0, S=0)
